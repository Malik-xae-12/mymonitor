...
from backend.app.models.monitoring import PipelineRun, ActivityRun, ActivityError
...
class HierarchyTreeBuilder:
    ...
    def parse_activity(raw_act: Dict[str, Any]) -> ActivityRun:
        ...
        if raw_error and isinstance(raw_error, dict) and (raw_error.get("errorCode") or raw_error.get("message")):
            ...
            # Fallback if error is in output
    ...
    def assemble_pipeline_tree(
        ...
        for r in raw_runs:
            ...
            # Detect child runs spawned by ExecutePipeline / InvokePipeline activity
            for act in raw_activities:
                ...
                if any(kw in act_type for kw in ["executepipeline", "invokepipeline", "pipeline"]):
                    ...
                    # Check for child pipeline reference (ID or name)
                    child_ref = (
        ...
        # 2. Dynamically link child pipelines to parents
        # Pass 1: Direct link by run ID, pipeline ID, or pipeline name
        for run_id, run_obj in runs_by_id.items():
            ...
            if parent_id and parent_id in runs_by_id and parent_id != run_id:
                ...
                # Attach directly to matching activity inside parent
                attached = False
        ...
        # 3. Assemble top-level runs (ONLY true root pipelines, NEVER sub-pipelines!)
