...
class AlertService:
    ...
    async def process_failed_run(
        ...
        existing = await db_service.get_active_incident_for_run(pipeline_run_id)
        if existing:
        ...
        sla_cfg = await db_service.get_sla_config(workspace_id, pipeline_id)
        ...
        await db_service.create_incident(incident)
    ...
    <div class="footer">
      ...
      If unresolved before SLA expires ({sla_minutes}m), automatic L2 escalation triggers.
        ...
        if sent:
            ...
            await db_service.update_incident(incident_id, {"l1_notified_at": now_str, "updated_at": now_str})
    ...
    async def _sla_monitor_loop(self):
        ...
        while self._is_running:
            try:
                ...
                active_incidents = await db_service.get_all_unresolved_incidents()
                now_utc = datetime.datetime.now(datetime.timezone.utc)
                ...
                for inc in active_incidents:
                    ...
                    if now_utc >= target_dt:
                        ...
                        sla_cfg = await db_service.get_sla_config(ws_id, p_id)
  ...
  <div class="card">
    ...
    <div class="footer">
      Microsoft Fabric Real-Time Monitoring Hub • Automated L2 Escalation System
                        ...
                        await db_service.update_incident(inc_id, {
                            "status": "ESCALATED_L2",
    ...
    async def resolve_incident(self, incident_id: str, resolved_by: str = "User") -> Optional[Dict[str, Any]]:
        ...
        now_str = datetime.datetime.now(datetime.timezone.utc).isoformat()
        await db_service.update_incident(incident_id, {
            "status": "RESOLVED",
            "resolved_at": now_str,
    ...
    async def rearm_and_notify_l1(self, workspace_id: str, pipeline_id: str, l1_email: str, sla_minutes: int):
        ...
        """
        history = await db_service.get_pipeline_history(workspace_id, pipeline_id)
        if not history:
            return
        latest_run = history[0]
        if latest_run.get("status", "").lower() != "failed":
            return
